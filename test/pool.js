...
contract('LiquidityPool', async (accounts) => {
    ...
    let weth; let mkr; let dai; let
        xxx; // TTokens
    let factory; // LiquidityPool factory
    ...
    before(async () => {
        ...
        POOL = await factory.newLiquidityPool.call();
    ...
    describe('LpToken interactions', () => {
        it('Token descriptors', async () => {
            const name = await pool.name();
            assert.equal(name, 'Desyn Pool Token');

            const symbol = await pool.symbol();
            assert.equal(symbol, 'DPT');

...
contract('Factory', async (accounts) => {
    ...
    describe('Factory', () => {
        ...
        before(async () => {
            ...
            POOL = await factory.newLiquidityPool.call(); // this works fine in clean room
            await factory.newLiquidityPool();
            pool = await LiquidityPool.at(POOL);
        ...
        it('isLiquidityPool on non pool returns false', async () => {
            const isLiquidityPool = await factory.isLiquidityPool(admin);
            assert.isFalse(isLiquidityPool);
        });

        it('isLiquidityPool on pool returns true', async () => {
            const isLiquidityPool = await factory.isLiquidityPool(POOL);
            assert.isTrue(isLiquidityPool);
        });
